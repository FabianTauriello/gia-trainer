    ...
    <div className="flex justify-center gap-3 mx-20">
      {choices.map((choice, i) => (
        <button
          key={i}
          type="button"
          onClick={quiz.inReview ? undefined : () => handleAnswerClick(i)}
          className={`outline-none p-6 bg-secondary rounded-lg flex-1 ${
            hideChoices || quiz.inReview ? "cursor-default" : "cursor-pointer"
          } ${correctChoice === i ? "border-4 border-white" : ""}`}
        >
          <span className={`${hideChoices && !quiz.inReview ? "invisible" : "visible"}`}>{choice}</span>
        </button>
      ))}
    </div>
  );
